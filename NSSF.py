...
from NSlice import NSlice, UENS
...
"""Database creation"""
def getSliceroot():
    Slicesdb = DataBase('NS.config') #holding Slice Object Information
    Sliceroot = Slicesdb.dbroot #Network Slice DB object
    return Sliceroot

def getUESRoot():
      #Creates a UEId and NSId that represent connection information
     UESlice = DataBase('Table.config') #for keeping Connection Information
...
def RecieveUEInfo(UESRoot, MDDVector):
...
#            print "Please use only Numbers"
    ...
    Bigio = "UENS" + str(MDDVector.UEId) #Comparission
#    i = let_user_pick(ServiceType)#Calls a function for selecting Type of Service
    i = MDDVector.ServiceType
    try:
        UENService = UENS(ServiceType[i-1], MDDVector.UEId )
    except:
        UENService = UENS("NotFound", MDDVector.UEId)
    ...
    if count > 0:
        ...
        RegisterUENSId(MDDVector.UEId, MDDVector.NSId, UESRoot) #Registers Local Database
